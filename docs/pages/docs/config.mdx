...
ESLint can be configured to warn when using components that have Adaptive counterparts
by adding the following to the `rules` section in `eslint.config.mjs`.
...
Specific components can be excluded by passing a list of names into `warnNonAdaptiveComponents`.

```tsx pure
import { warnNonAdaptiveComponents } from "adaptive-material-ui/eslintRule";

rules: {
  "no-restricted-syntax": ["warn", ...warnNonAdaptiveComponents(["Button"])],
}
```
