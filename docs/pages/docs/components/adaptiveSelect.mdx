...
- The `option` element [browser support](https://caniuse.com/?search=base-select) &
...
using the `disableNativeEmptyValue` prop if needed.
...
- Props not applicable to a `<option>` element are ignored on native.
...
- Props not applicable to a `<optgroup>` element are ignored on native.
