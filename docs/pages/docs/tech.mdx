...
Each device type is split into it's own file use React [lazy](https://react.dev/reference/react/lazy) so a Android device won't load iOS styles.
The imports are destructured so only the referenced components are included in the build (tree shaking).
